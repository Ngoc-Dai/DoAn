<template>
  <nav class="home-nav">
    ...
    <div class="home-nav__menu">
      <ul class="nav-menu">
        <li v-for="(item, index) in listCollection" :key="index" class="nav-menu-item">
    ...
    <div class="home-nav__option">
      <form @submit.prevent="testSnackbar(searchTerm)" class="search-form">
      ...
      <div class="nav-option__item nav-option__item-login">
        ...
        <div class="menu-account">
          <div class="menu-account__item" @click="openLogoutUser" v-if="Object.keys(userLogin).length !== 0">
            Đăng xuất
          </div>

          <div class="menu-account__item" @click="openCartDetail" v-if="userLogin.role === 'CUSTOMER'">
            Tài khoản của bạn
          </div>

          <div class="menu-account__item" @click="openAdmin" v-if="userLogin.role === 'ADMIN'">
            Trang quản trị admin
          </div>

          <!-- chua dang nhap -->
          <div class="menu-account__item" @click="openLogin" v-if="Object.keys(userLogin).length === 0">
            Đăng nhập
          </div>

          <div class="menu-account__item" @click="openCreateAccount" v-if="Object.keys(userLogin).length === 0">
            Tạo tài khoản
          </div>
        </div>
      </div>
      ...
      <div class="nav-option__item nav-option__item-login" v-if="userLogin.role === 'CUSTOMER'">
...
export default {
  ...
  methods: {
    ...
    testSnackbar(searchTerm) {
      if (searchTerm) {
        this.$router.replace('/search/' + encodeURIComponent(this.searchTerm));
      } else {
        this.getSnackBars("Vui lòng nhập từ khóa tìm kiếm");
      }
    },
