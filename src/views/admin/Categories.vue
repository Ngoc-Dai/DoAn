...
export default {
  ...
  methods: {
    ...
    async getCategoryDataBySlug(item) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/category/${item.slug}`
        );
    ...
    editCategory() {
      ...
      axios
        .put(
          `http://localhost:5000/category/${me.categoryDelete.id}`,
          me.categoryDelete
        )
    ...
    addCategory() {
      ...
      if (this.categoryByID.name === "" || this.categoryByID.slug === "") {
        ...
        axios
          .post("http://localhost:5000/category", this.categoryByID)
          .then(function(response) {
    ...
    deleleCategoryByID() {
      ...
      const me = this;
      axios
        .delete(`http://localhost:5000/category/${me.categoryDelete.id}`)
        .then(function(response) {
