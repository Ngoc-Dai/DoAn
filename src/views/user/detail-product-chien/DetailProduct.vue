  ...
  <div class="list-product">
    ...
    <div class="content">
      <div class="image-product">
        <div class="img">
          <img :src="require('../../../assets'.concat(product.avatar))" alt="" />
        </div>
      </div>
      <div class="deltail">
        <p>{{ product.name }}</p>
        ...
        <div class="acction">
          <div class="addToCart" @click="addProductToCart">
            Thêm vào giỏ hàng
            <span>
              <img id="cart" src="../../../assets/img/icon/card-white.png" alt="" /></span>
          </div>
          <div class="buyNow">Mua ngay</div>
          <div class="addToWishList">
            <span>
              <img id="heart" src="../../../assets/img/icon/heart.png" alt="" /></span>
            Sản phẩm yêu thích
          </div>
        </div>
      </div>
    ...
    <div class="related">
      ...
      <div class="list-related">
        <ul>
          <li class="product" v-for="item in productRelated" :key="item.id" @click="viewDetail(item)">
            <div>
  ...
  methods: {
    ...
    async getData() {
      const response = await axios.get(
        "http://localhost:5000/product/" + this.slug
      );
      this.product = response.data;
      // console.log(this.product.originPrice);
      const relateds = await axios.get("http://localhost:5000/product/all");
